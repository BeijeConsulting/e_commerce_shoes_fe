...
function MobileMenu(props) {
    ...
    if (item.bottom === false) {
      ...
      return (
        <li key={`${key}-${Math.random()}`}>
          {state.active === null && (
            <div onClick={goTo(path)} className={"mobile-menu__item"}>
              <div>{item.top}</div>
              <ArrowForwardIosIcon />
            </div>
          )}
            ...
            <>
              <div
                ...
                className={`mobile-menu__item ${
                  state.active === item.path ? "active" : ""
                }`}
              >
                <div onClick={setActive} data-menu-path={item.path}>
                  {item.top}
                </div>
                ...
                {state.active === item.path && (
                  <div className="mobile-menu__item--clicked">
                    <div
                      className="mobile-menu__item--clicked__all"
                      onClick={goTo(`${path}`)}
                    >
                      TUTTE
                    </div>
  ...
  function notifyLogOutError() {
    toast.error(t("toastify.mobileMenu.logoutError"), {
  ...
  function notifyLogOutSuccess() {
    toast.success(t("toastify.mobileMenu.logoutSuccess"), {
      position: toast.POSITION.TOP_CENTER,
      autoClose: 500,
    });
  }
              ...
              {!!user.isLogged ? (
                <>
                  <div className="main-header__mobile-menu__bottom__user-info">
                    <Avatar sx={{ bgcolor: "#4f4f4f" }} onClick={goToAccount}>
                      {initials}
                    </Avatar>
                    <div
                      className="main-header__mobile-menu__bottom__user-info__info"
                      onClick={goToAccount}
                    >
                ...
                <>
                  <Button
                    label={t("mobileMenu.signin")}
                    buttonStyle={"default-button full-width"}
                    handleClick={goToSignin}
                  />
                  <p
                    className="main-header__mobile-menu__bottom__text"
                    onClick={goToSignup}
                  >
                    {t("mobileMenu.signup")}
                  </p>
